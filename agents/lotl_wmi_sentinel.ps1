...
Write-Host "📍 Active Non-Admin Process Ingress & Command-Line Heuristic Engine Hooked" -ForegroundColor Yellow

$KnownPids = [System.Collections.Generic.HashSet[int]]::new()
Get-CimInstance Win32_Process | ForEach-Object { $KnownPids.Add($_.ProcessId) | Out-Null }

Write-Host "👀 Monitoring process creation in real-time. Press Ctrl+C to stop.`n" -ForegroundColor Green
...
while ($true) {
    ...
    foreach ($proc in $currentProcesses) {
        if (-not $KnownPids.Contains($proc.ProcessId)) {
            $KnownPids.Add($proc.ProcessId) | Out-Null
